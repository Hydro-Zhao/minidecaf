...
// TODO
// C++中规定只要返回值不为void的函数没有return，不管返回值有没有被使用，都是未定义行为
...
antlrcpp::Any Visitor::visitProgram(MiniDecafParser::ProgramContext *ctx) {
  ...
  for (auto i : ctx->declaration()) {
    if (symbol_table.back().find(i->Identifier()->getText()) != symbol_table.back().end())
    ...
    if (i->Lbrkt().size() != 0) {
      ...
      variable.type = -1;
      int size = 1;
      for (auto j : i->Integer()) {
        // TODO step12 每一维长度只能是正整数常数，不能是零或负数
        variable.length_list.push_back(std::stoi(j->getText()));
        size *= std::stoi(j->getText());
      }
      variable.size = size;
      local_index += size - 1;
    } else {
      if (i->Equal()) {
        ...
        // 需要检查是否越界
        // 这里Integer是vector因为有可能是数组
        // TODO 添加对数组的支持
        std::string integer{i->Integer().begin()->getText()};
        ...
        variable.init = std::stoi(i->Integer().begin()->getText());
      }
    }
...
antlrcpp::Any
Visitor::visitInteger(MiniDecafParser::IntegerContext *ctx) {
...
antlrcpp::Any Visitor::visitUnaryOp(MiniDecafParser::UnaryOpContext *ctx) {
  ...
  if (ctx->Minus()) {
    ...
    code.push_back("neg");
    return NodeInfo{type : unaryinfo.type, is_lvalue : false};
    ...
    return NodeInfo{type : unaryinfo.type + 1, is_lvalue : false};
  } else if (ctx->Asterisk()) {
    if (unaryinfo.type == 0)
      ERROR("* must operate on a pointer");
    code.push_back("load");
    // 操作数类型 e 是指针类型 T*，结果类型是 T
    return NodeInfo{type : unaryinfo.type - 1, is_lvalue : true};
  }
}

// step3, step12
antlrcpp::Any Visitor::visitAddSub(MiniDecafParser::AddSubContext *ctx) {
  NodeInfo loperand = visit(ctx->additive());
  NodeInfo roperand = visit(ctx->multiplicative());
  if (ctx->Plus()) {
...
antlrcpp::Any Visitor::visitMulDiv(MiniDecafParser::MulDivContext *ctx) {
  ...
  if (ctx->Asterisk()) {
    code.push_back("mul");
    return NodeInfo{type : lvalue.type, is_lvalue : false};
  } else if (ctx->Slash()) {
...
antlrcpp::Any
Visitor::visitDeclaration(MiniDecafParser::DeclarationContext *ctx) {
  if (symbol_table.back().find(ctx->Identifier()->getText()) != symbol_table.back().end())
    ERROR("Duplicated declaration");
  Var variable;
  ...
  if (ctx->Lbrkt().size() == 0) {
    if (ctx->Equal()) {
      ...
      // step5 实验指导 遇到表达式语句时，生成完表达式的 IR 以后记得再生成一个
      // pop
      code.push_back("pop");
    }
    paranum++;
    ...
    variable.type = -1;
    int size = 1;
    for (auto i : ctx->Integer()) {
      // TODO step12 每一维长度只能是正整数常数，不能是零或负数
      variable.length_list.push_back(std::stoi(i->getText()));
      size *= std::stoi(i->getText());
    }
    variable.size = size;
    local_index += size - 1;
    paranum += size;
  ...
  return NodeInfo{type : variable.type, is_lvalue : true};
}

// 这里默认生成的是右值，如果需要地址自行添加pop指令（基本只有&操作符需要）
// TODO 添加数组访问以及返回含数组basetype的nodeinfo
antlrcpp::Any
Visitor::visitIdentifier(MiniDecafParser::IdentifierContext *ctx) {
...
antlrcpp::Any
Visitor::visitAssign(MiniDecafParser::AssignContext *ctx) {
  ...
  code.push_back("store");
  return NodeInfo{type : lvalue.type, is_lvalue : true};
}

antlrcpp::Any
Visitor::visitSingleExpr(MiniDecafParser::SingleExprContext *ctx) {
  visit(ctx->expression());
  // step5 实验指导 遇到表达式语句时，生成完表达式的 IR 以后记得再生成一个 pop
  code.push_back("pop");
}
...
antlrcpp::Any Visitor::visitIfStmt(MiniDecafParser::IfStmtContext *ctx) {
  std::string end_label = "IF_LABEL_" + std::to_string(if_order++);
    ...
    std::string else_label = "IF_LABEL_" + std::to_string(if_order++);
        code.push_back("beqz "+ else_label);
...
// step7
antlrcpp::Any Visitor::visitCompound_statement(
    MiniDecafParser::Compound_statementContext *ctx) {
...
antlrcpp::Any Visitor::visitForLoop(MiniDecafParser::ForLoopContext *ctx) {
  std::string beginloop_label =
      "BEGINLOOP_LABEL_" + std::to_string(loop_order++);
...
antlrcpp::Any
Visitor::visitForLoopDec(MiniDecafParser::ForLoopDecContext *ctx) {
  std::string beginloop_label =
      "BEGINLOOP_LABEL_" + std::to_string(loop_order++);
...
// step9, step10
antlrcpp::Any
Visitor::visitFunction(MiniDecafParser::FunctionContext *ctx) {
  ...
  if (symbol_table.front().find(ctx->Identifier()->getText()) != symbol_table.front().end())
    ERROR("function name violate with a global variable");
  if (func_table.find(ctx->Identifier()->getText()) != func_table.end()) {
    Func f;
  ...
  if (func_table[ctx->Identifier()->getText()].framesize == -1) {
    ...
    code.push_back(std::string("label FUNC_") + ctx->Identifier()->getText());
    ...
    for (int i = 0; i < ctx->parameter_list()->Identifier().size(); ++i) {
      ...
      f.parameter_list.push_back(
          std::make_pair(visit(ctx->parameter_list()->type(i)), funcname));
...
antlrcpp::Any
Visitor::visitFuncCall(MiniDecafParser::FuncCallContext *ctx) {
  ...
  for (auto i = ctx->expression_list()->expression().crbegin();
    ...
    NodeInfo param = visit(*i);
    if (param.type != f.parameter_list[j].first)
      ERROR("function call parameter type error");
  }
  code.push_back(std::string{"call FUNC_"} + ctx->Identifier()->getText());
...
antlrcpp::Any Visitor::visitCast(MiniDecafParser::CastContext *ctx) {
  // TODO step11
  NodeInfo unaryinfo = visit(ctx->unary());
  return NodeInfo{type: visit(ctx->type()), is_lvalue: unaryinfo.is_lvalue};
}
...
antlrcpp::Any Visitor::visitType(MiniDecafParser::TypeContext *ctx) {
  ...
  // （目前）只有int***这种情况
    ...
    if (ctx->type())
      // 很不Modern C++的类型转换
      return type + (int)visit(ctx->type());
    else
      return 1;
  }
}
...
// TODO 注意，这里判断不要写 e1.type == PointerType, 而要写 e1.type instanceof
// PointerType?
