...
public:
  ...
  // step5
  antlrcpp::Any
  visitDeclaration(MiniDecafParser::DeclarationContext *ctx) override;
  antlrcpp::Any
  visitIdentifier(MiniDecafParser::IdentifierContext *ctx) override;
  antlrcpp::Any visitAssign(MiniDecafParser::AssignContext *ctx) override;
  antlrcpp::Any
  visitSingleExpr(MiniDecafParser::SingleExprContext *ctx) override;
  // step6
  ...
  // step7
  antlrcpp::Any visitCompound_statement(
      MiniDecafParser::Compound_statementContext *ctx) override;
  // step8
  antlrcpp::Any visitForLoop(MiniDecafParser::ForLoopContext *ctx) override;
  antlrcpp::Any
  visitForLoopDec(MiniDecafParser::ForLoopDecContext *ctx) override;
...
private:
  int find_symbol(std::string name) {
    int index = 0;
    for (auto i = symbol_table.rbegin(); i != symbol_table.rend(); --i) {
      for (auto j = i->begin(); j != i->end(); ++j)
        index += j->second.size;
      if (i->find(name) != i->end())
        return index - (*i)[name].index;
    }
    return -1;
  }
...
public:
  ...
  std::map<std::string, Func> get_func_table() { return func_table; }
};
